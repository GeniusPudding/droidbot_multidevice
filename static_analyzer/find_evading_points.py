...
startmethodsign_from_log = lambda line: line[line.index('Method START')+22: line.index(' $(')] \
    if line.startswith('Method START(target)') else line[line.index('Method START')+14: line.index(' $(')] 
endmethodsign_from_log = lambda line: line[line.index('Method END')+20: line.index(' $(')] \
    if line.startswith('Method END(target)') else line[line.index('Method END')+12: line.index(' $(')]
branchmethodsign_from_log = lambda line: line[line.index('Branch')+16: line.find('->', 2)] \
    if line.startswith('Branch(Switch): ') else get_tag_sign(line) + line[line.index('Branch')+8: line.index('->if')]
calleesign_from_log = lambda line: line.split('=>')[1].split('(')[0]
casesign_from_log = lambda line: get_tag_sign(line) + line[5:line.index(':')]
gotosign_from_log = lambda line: get_tag_sign(line) if line.startswith('Goto Tag') else line[6:].split(' $(')[0].replace(' :', ' ')
trysign_from_log = lambda line: line.split(') :')[1].split(' $(')[0].replace(':', '') if line.startswith('Try Done') else get_tag_sign(line)
...
def gen_dynamic_callgraph(path, logs,api_tree, new_version = True):#希望支援多線程
    dynamic_call_map = {'call_seqs':[],'cur_calling_method':[], 'calling_stacks':[], 'called_by':{}, 'id_sign': {}}
    ...
    if new_version:
        ...
        for i,line in tqdm(enumerate(logs)):
            ...
            if len(relation) == 0:
                #input(f'line:{line} 沒有ID???')
                if 'Malicious Behavior' in line:
                    continue
                else:
                    raise ValueError(f'line:{line} 沒有ID???')
            this_method_id = relation[0]#call relation不用這個
            #print(f'this_method_id:{this_method_id}, line:{line}, cur_calling_method:{cur_calling_method}')
            if line.startswith('CALL Relation'):
                caller_id, callee_id = None, relation[-1]
                if len(relation) == 2: caller_id = relation[0]
                ...
                #開啟新的seq，或是抓出calling relation findall
                # if '000000001' in line:  
                #     print(f'000000001 id_sign:{id_sign}, caller:{caller}, callee:{callee}, caller_id:{caller_id}, callee_id:{callee_id}')
                if '(No Caller)' in line or '<clinit>' in caller or (caller_id and caller_id in id_sign and id_sign[caller_id] != caller):  #這串都是要當seq的頭的
                    #<clinit>代表某函數執行到一半因為要呼叫別的class method而去init class
                    #
                    ...
                    called_by[i] = 'entry'
                    cur_calling_method.append(new_id)
                elif not '(No Caller)' in line and not '<clinit>' in caller and caller_id and caller_id not in id_sign:#可能因為multi thread原因錯誤蓋掉ID導致找不到開頭的 
                    #print(f'i:{i}, line:{line} 沒有caller, id_sign:{id_sign}, caller:{caller}')
                    continue
                else:
                    _test_start = False
                    ...
                    for ii, cur_id in enumerate(cur_calling_method):
                        ...
                        if cur_id == caller_id:#某一seq的下一個call
                            called_by[i] = calling_stacks[ii][-1] 
                            calling_stacks[ii].append(i)
                            call_seqs[ii].append(i)
                            cur_calling_method[ii] = callee_id#後面交給Method Start 
                            #print(f'caller_id:{caller_id}, callee_id:{callee_id}, cur_calling_method:{cur_calling_method}')
                            _test_start = True 
                            break
                             
                    # if not _test_start:
                    #     input(f'i:{i}, line:{line} 找不到Seq, calling_stacks:{calling_stacks},cur_calling_method:{cur_calling_method}, path:{path}') #不曉得為何有logcat錯誤 忽略 
                ...
                if this_method_id not in id_sign: #出現沒有被call rel log到的ID
                    #input(f'this_method_id:{this_method_id}, id_sign:{id_sign}')
                    continue
        ...
        #cs = dynamic_call_map['call_seqs']
        #print(f'dynamic_call_map[\'call_seqs\']:{cs}')
...
def multiple_seq_matching(log_name, diffs_basepath, real_seq_head,emu_seq_head, origin_r_logs, origin_e_logs, origin_real_callseqs, origin_emu_callseqs, layer = 0):
    ...
    for head_sign in real_seq_head:
        ...
        if len(real_call_seq) > 1 or len(emu_call_seq) > 1:#real或emu有多個seq以這個method sign開頭，real_call_seq是list of seq
            ...
            #print(f'next_real_seq_head:{next_real_seq_head}, next_emu_seq_head:{next_emu_seq_head}')
...
def gen_log_seq_legalpath(cat, head_sign, count=None):#cat: 'realseq', 'emuseq' 
    ...
    head_sign = head_sign.replace('<init>', 'init')
    if count:
        #print(f'head_sign:{head_sign},sign_to_legalpathstr(head_sign):{sign_to_legalpathstr(head_sign)}')
        return os.path.join(diff_dir,log_name,cat,sign_to_legalpathstr(head_sign)+'('+str(count)+').txt')
    count = 0 
    while True:
        #print(f'while head_sign:{head_sign},sign_to_legalpathstr(head_sign):{sign_to_legalpathstr(head_sign)}, count:{count}')
...
def get_evading_index(diffs_basepath, real_callseqs, emu_callseqs):
    ...
    for diff in os.listdir(diffs_basepath):
        ...
        #print(f'r_index_seq:{r_index_seq},e_index_seq:{e_index_seq}')
        for diff_line in r:#裡面全都要用r_index_seq, e_index_seq來做index轉換
            try:
                if 'a' in diff_line:
                    ...
                    # input(f'diff_left:{diff_left},diff_right:{diff_right}') 
...
def main(log_name,p2f):
    ...
    api_tree = {}
    real_parent_index, real_callseqs, real_cur_calling_method = gen_dynamic_callgraph(real_log_path, rr,api_tree)
    emu_parent_index, emu_callseqs, emu_cur_calling_method  = gen_dynamic_callgraph(emu_log_path, er,api_tree)
    print('dynamic_callgraph Done')
    #把那些還沒終結的callseqs過濾掉 合理嗎?
    
    #可以先來一個real_callseqs跟emu_callseqs逐項對比的部分 假設正常情況會是1-1, 2-2, ...這樣嗎
    #print(f'real_callseqs:{real_callseqs},emu_callseqs:{emu_callseqs}')
    #print(f'rr:{rr},er:{er},len real_callseqs:{len(real_callseqs)},len emu_callseqs:{len(emu_callseqs)}')
