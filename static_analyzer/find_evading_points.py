...
diff_dir = 'C:\\Users\\user\\Desktop\\testing\\dataset\\diffs'
ag_dir = 'C:\\Users\\user\\Desktop\\droidbot_multidevice\\static_analyzer'
log_path = 'C:\\Users\\user\\Desktop\\testing\\dataset\\\method_seq_logs\\RealJ6+_SamsungGalaxyS10\\TriggerZoo_antiemulator_withend'
...
def gen_cfg_info(logs,evading_index,parent_index):
...
def get_evading_point(method_jsonObj):#TODO 需要實際evading案例來Debug
    ...
    d = method_jsonObj['cfg']['toparent']
    if d[block_offset] == []:
        input('Start diverge before 0x0 block, why?')
    else:
...
if __name__ == '__main__':
    ...
    for i in real_evading_index:
        ...
        if p_i == 'entry':#p_i == 'index'是原因不明的差異
            continue
        method_obj = gen_cfg_info(rr,i,p_i) 
        if not method_obj:
            continue   
        #然後抓出branch的位子 
        get_evading_point(method_obj)
    ...
    for i in emu_evading_index:
        ...
        if p_i == 'entry':
            continue
        method_obj = gen_cfg_info(er,i,p_i)  
        if not method_obj:
            continue  
        input(f'p_i:{p_i}\nmethod_obj:{method_obj}')
        get_evading_point(method_obj)
